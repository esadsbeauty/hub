...
for (const file of files) {
  ...
  if (operations.length) {
    const review = sql.match(/^-- db-audit: reviewed-destructive ([a-z0-9-]+)$/m)?.[1];
    (review ? reviewedDestructive : destructive).push({ file, operations, ...(review ? { review } : {}) });
  }
}
console.log(JSON.stringify({ migrations: files.length, ordered: files, destructive, reviewedDestructive }, null, 2));
