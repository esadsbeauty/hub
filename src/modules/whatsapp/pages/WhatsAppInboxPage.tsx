...
export function WhatsAppInboxPage(){
  ...
  const selected=inbox.data?.conversations.find(
  ...
  const canReply=
    isPlatformAdmin||
    ["owner","admin","manager","sales","operations","marketing"].includes(role);
  ...
  const filtered=useMemo(()=>{
    ...
    return(inbox.data?.conversations??[]).filter(
      item=>
  ...
  if(!inbox.data?.connection)
    return(
      <div className="grid min-h-[60dvh] place-items-center text-center">
        <div className="max-w-md">
          <span className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-champagne-soft text-champagne-dark">
            <MessageCircle/>
          </span>
          ...
          <p className="mt-3 leading-7 text-muted-foreground">
            Conecte uma conta do WhatsApp Business para receber conversas diretamente no CRM.
          </p>
          ...
          <p className="mt-2 text-xs text-muted-foreground">
            Configuração disponível em uma próxima etapa.
          </p>
        </div>
      </div>
    );
  ...
  return(
    <div className="-mx-4 -my-6 min-[430px]:-mx-5 md:-mx-6 md:-my-8 lg:-mx-8">
      <div className="grid h-[calc(100dvh-4rem)] max-h-[calc(100dvh-4rem)] min-h-0 overflow-hidden border-y bg-card md:grid-cols-[20rem_minmax(0,1fr)] xl:grid-cols-[20rem_minmax(0,1fr)_19rem]">
