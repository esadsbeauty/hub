...
export type LostReason = 'price' | 'no_response' | 'no_interest' | 'competitor' | 'timing' | 'no_budget' | 'unqualified' | 'other';
...
export type Opportunity = { id: string; organizationId: string; companyId: string; pipelineId: string; stageId: string; title: string; description?: string; value: number; probability: number; expectedCloseDate?: string; ownerId?: string; owner?: string; source?: string; status: OpportunityStatus; lostReason?: LostReason; lostReasonNotes?: string; wonAt?: string; lostAt?: string; createdBy: string; createdAt: string; updatedAt: string; stageEnteredAt: string; deletedAt?: string | null };
export type OpportunityStageHistory = { id: string; organizationId: string; opportunityId: string; fromStageId?: string; toStageId: string; changedBy: string; changedAt: string };
...
export type CompanyNote = { id: string; organizationId: string; companyId: string; opportunityId?: string; text: string; author: string; createdAt: string; updatedAt: string };
export type CrmData = { organization: Organization; profile: Profile; companies: Company[]; contacts: CompanyContact[]; pipelines: Pipeline[]; stages: PipelineStage[]; opportunities: Opportunity[]; stageHistory: OpportunityStageHistory[]; events: TimelineEvent[]; tasks: Task[]; activities: CommercialActivity[]; files: CompanyFile[]; notes: CompanyNote[] };
