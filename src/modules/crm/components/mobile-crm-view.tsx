...
function MobileOpportunityCard({item,stage,company,contact,task,open,move}:{item:Opportunity;stage:PipelineStage;company?:Company;contact?:CompanyContact;task?:Task;open:()=>void;move:()=>void}){const phone=contact?.whatsapp??contact?.phone??company?.whatsapp??company?.phone,whatsapp=contactWhatsappUrl(phone),leadName=contact?.name??company?.responsibleName??item.title;return <Card className="ring-1 ring-black/[.025]"><CardContent className="p-4"><button className="w-full text-left" onClick={open}><h3 className="text-xl font-semibold leading-snug tracking-[-.025em]">{leadName}</h3>{item.title!==leadName&&<p className="mt-1 text-sm text-muted-foreground"><span className="font-medium text-foreground">Interesse:</span> {item.title}</p>}{phone&&<p className="mt-2 text-[15px] font-medium">{phone}</p>}{item.owner&&<p className="mt-2 text-sm text-muted-foreground">Responsável: {item.owner}</p>}{task&&!stage.isWon&&!stage.isLost&&<div className="mt-3 rounded-xl border border-champagne/50 bg-champagne-soft/60 p-3 text-sm"><p className="font-semibold">Próxima ação: {task.title||"Follow-up"}</p><p className="mt-1 text-muted-foreground">{formatDateTime(task.dueAt)}</p></div>}</button><div className="mt-3 grid grid-cols-3 gap-2 border-t pt-3">{whatsapp?<a aria-label="Abrir WhatsApp do contato" className="grid min-h-12 place-items-center rounded-xl bg-[#e8f7ee] text-[#176b3a] premium-focus" href={whatsapp} target="_blank" rel="noreferrer"><MessageCircle size={21}/></a>:<span aria-label="WhatsApp não informado" className="grid min-h-12 place-items-center rounded-xl bg-muted text-muted-foreground opacity-50"><MessageCircle size={21}/></span>}<button aria-label="Abrir oportunidade" className="grid min-h-12 place-items-center rounded-xl bg-muted premium-focus" onClick={open}><CalendarPlus size={21}/></button><button className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-primary px-3 text-sm font-semibold text-primary-foreground premium-focus" onClick={move}><MoveRight size={19}/>Mover</button></div></CardContent></Card>}
