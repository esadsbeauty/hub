...
export function MobileCrmView(props: Props) {
  ...
  return <div className="space-y-8 md:hidden">
    <header><p className="text-base font-medium text-muted-foreground">Gestão comercial</p><h1 className="mt-1 text-[2.25rem] font-bold leading-none tracking-[-.055em]">CRM</h1><p className="mt-3 text-lg leading-7 text-muted-foreground">Empresas, oportunidades e próximos passos.</p></header>
    <section aria-label="Indicadores do CRM" className="-mx-5 flex snap-x snap-mandatory gap-4 overflow-x-auto scroll-smooth px-5 pb-2">
    ...
    <section className="space-y-4">
      <div className="grid grid-cols-2 rounded-2xl bg-muted p-1.5">
        <Button variant={view==="kanban"?"default":"ghost"} onClick={()=>setView("kanban")}><Kanban size={20}/>Kanban</Button>
        <Button variant={view==="list"?"default":"ghost"} onClick={()=>setView("list")}><List size={20}/>Lista</Button>
      </div>
      <SearchInput value={props.query} onChange={event=>props.onQueryChange(event.target.value)} placeholder="Buscar lead ou contato…"/>
      <div className="grid grid-cols-2 gap-3"><Button variant="outline" onClick={()=>setFilterSheet(true)}><SlidersHorizontal size={20}/>Filtros{props.activeFilters>0&&` (${props.activeFilters})`}</Button><Button variant="outline" onClick={()=>setSortSheet(true)}><ArrowDownUp size={20}/>{sortLabel[props.sort]}</Button></div>
    </section>
...
function MobileMetric({label,value,hint}:{label:string;value:number;hint:string}) { return <div className="min-w-[84vw] snap-center"><MetricCard label={label} value={value} hint={hint}/></div>; }
...
function MobileCompanyList(props:Props){return <section className="grid gap-4" aria-label="Empresas em lista">{props.companies.map(company=>{const contact=props.contacts.find(item=>item.companyId===company.id&&item.isPrimary);const companyOpportunities=props.opportunities.filter(item=>item.companyId===company.id&&item.status==="open");const task=companyOpportunities.map(props.nextTask).filter((item):item is Task=>Boolean(item)).sort((a,b)=>a.dueAt.localeCompare(b.dueAt))[0];return <button key={company.id} onClick={()=>props.onOpenCompany(company)} className="rounded-[1.5rem] bg-card p-5 text-left shadow-soft premium-focus"><div className="flex items-start justify-between gap-3"><div><h2 className="text-xl font-semibold tracking-[-.025em]">{company.fantasyName}</h2><p className="mt-1 text-base text-muted-foreground">{contact?.name??"Sem contato principal"}</p></div><span className="rounded-full bg-muted px-3 py-1.5 text-sm font-semibold">{props.opportunities.filter(item=>item.companyId===company.id&&item.status==="open").length} abertas</span></div><div className="mt-5 flex flex-wrap gap-2"><TemperatureBadge temperature={company.temperature}/><PriorityBadge priority={company.priority}/></div><div className="mt-5 border-t pt-4 text-base"><p>{company.whatsapp??company.phone??"Contato não informado"}</p><p className="mt-2 text-muted-foreground">Próximo passo: {task?formatDateTime(task.dueAt):"Nenhum"}</p></div></button>})}</section>}
...
function MobileKanban({stageFilter,...props}:Props&{stageFilter:string}){
 ...
 return <section className="space-y-4"><div className="rounded-2xl bg-card p-4 shadow-soft"><div className="flex items-center justify-between gap-3"><div><p className="text-sm font-semibold uppercase tracking-[.12em] text-muted-foreground">Etapa do pipeline</p><p className="mt-1 text-lg font-semibold">{ordered.find(item=>item.id===first)?.name??"Pipeline"}</p></div><span className="text-sm text-muted-foreground">{Math.max(1,visible.findIndex(item=>item.id===first)+1)} de {visible.length}</span></div><Select className="mt-3" aria-label="Ir para etapa" value={first} onChange={event=>jump(event.target.value)}>{visible.map(stage=><option key={stage.id} value={stage.id}>{stage.name}</option>)}</Select></div><div className="-mx-5 flex snap-x snap-mandatory gap-4 overflow-x-auto scroll-smooth px-5 pb-5" aria-label="Pipeline por etapas">{visible.map((stage,index)=>{const rows=props.opportunities.filter(item=>item.stageId===stage.id);return <section id={`mobile-stage-${stage.id}`} key={stage.id} onFocus={()=>setFirst(stage.id)} className="min-w-[90vw] max-w-[24rem] snap-center rounded-[1.75rem] bg-muted/55 p-5"><header className="mb-5 border-b pb-4"><div className="flex items-start justify-between gap-3"><div><h2 className="text-[1.375rem] font-semibold">{stage.name}</h2><p className="mt-1 text-base text-muted-foreground">{rows.length} {rows.length===1?"oportunidade":"oportunidades"} · etapa {index+1}</p></div><span className="rounded-full bg-card px-3 py-1.5 text-base font-semibold shadow-soft">{rows.length}</span></div><p className="mt-3 text-xl font-semibold">{currency.format(rows.reduce((sum,item)=>sum+item.value,0))}</p></header><div className="space-y-4">{rows.length?rows.map(item=><MobileOpportunityCard key={item.id} item={item} company={props.companies.find(company=>company.id===item.companyId)} task={props.nextTask(item)} lastContact={props.lastContact(item)} stages={props.stages} open={()=>props.onOpenOpportunity(item)} move={()=>setMoving(item)}/>):<div className="rounded-2xl border border-dashed bg-card/70 p-6 text-center"><p className="text-lg font-semibold">Nenhuma oportunidade nesta etapa</p><p className="mt-2 text-base text-muted-foreground">Crie uma oportunidade ou mova uma negociação para cá.</p><Button className="mt-5 w-full" onClick={props.onCreateOpportunity}>+ Criar oportunidade</Button></div>}</div></section>})}</div><Modal open={Boolean(moving)} title="Mover etapa" onClose={()=>setMoving(undefined)}>{moving&&<div className="grid gap-3">{ordered.map(stage=><button key={stage.id} onClick={()=>{props.onMoveOpportunity(moving,stage.id);setMoving(undefined)}} className="flex min-h-16 items-center justify-between rounded-2xl border px-5 text-left text-base font-semibold premium-focus"><span>{stage.name}</span>{stage.id===moving.stageId&&<span className="text-sm text-muted-foreground">Atual</span>}</button>)}</div>}</Modal></section>;
}

function MobileOpportunityCard({item,company,task,lastContact,stages,open,move}:{item:Opportunity;company?:Company;task?:Task;lastContact?:TimelineEvent;stages:PipelineStage[];open:()=>void;move:()=>void}){return <Card className="ring-1 ring-black/[.025]"><CardContent className="p-5"><button className="w-full text-left" onClick={open}><p className="text-base font-medium text-muted-foreground">{company?.fantasyName??"Empresa"}</p><h3 className="mt-2 text-[1.3125rem] font-semibold leading-snug tracking-[-.025em]">{item.title}</h3><p className="mt-3 text-xl font-semibold">{currency.format(item.value)}</p><div className="mt-4 flex flex-wrap gap-2">{company&&<><TemperatureBadge temperature={company.temperature}/><PriorityBadge priority={company.priority}/></>}</div><p className="mt-3 text-base text-muted-foreground">{company?.whatsapp??"WhatsApp não informado"}</p><p className="mt-2 text-base text-muted-foreground">{item.owner??"Sem responsável"}</p><p className="mt-2 text-sm text-muted-foreground">Último contato: {lastContact?formatDateTime(lastContact.createdAt):"Nenhum registrado"}</p><NextActionStatus opportunity={item} stages={stages} task={task}/></button><div className="mt-5 grid grid-cols-3 gap-2 border-t pt-4">{company?.whatsapp?<a aria-label="Abrir WhatsApp" className="grid min-h-12 place-items-center rounded-xl bg-muted premium-focus" href={`https://wa.me/${company.whatsapp.replace(/\D/g,"")}`}><MessageCircle size={21}/></a>:<span aria-label="WhatsApp não informado" className="grid min-h-12 place-items-center rounded-xl bg-muted text-muted-foreground opacity-50"><MessageCircle size={21}/></span>}<button aria-label="Criar follow-up" className="grid min-h-12 place-items-center rounded-xl bg-muted premium-focus" onClick={open}><CalendarPlus size={21}/></button><button className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-primary px-3 text-sm font-semibold text-primary-foreground premium-focus" onClick={move}><MoveRight size={19}/>Mover</button></div></CardContent></Card>}
