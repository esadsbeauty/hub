...
export function OpportunityDetails(props: Props) {
  ...
  const contactRows=[["WhatsApp / telefone",rawPhone],["E-mail",contact?.email??company?.email],["Instagram",contact?.instagram??company?.instagram],["Cidade",company?.city]] as const;
  ...
  return <><Drawer open={props.open} title="Detalhe da oportunidade" onClose={props.onClose}><div className="space-y-5 overflow-x-hidden">
    ...
    <section><h3 className="text-xs font-bold uppercase tracking-[.14em] text-muted-foreground">Dados comerciais</h3><dl className="mt-3 grid grid-cols-2 gap-x-4 gap-y-3 text-sm">{commercial.map(([label,value])=><div key={label} className="border-b pb-2"><dt className="text-muted-foreground">{label}</dt><dd className="font-semibold">{value||"—"}</dd></div>)}</dl>{opportunity.description&&<p className="mt-3 rounded-xl bg-muted p-3 text-sm">{opportunity.description}</p>}<p className="mt-3 text-xs text-muted-foreground">Empresa vinculada: <b className="text-foreground">{company?.fantasyName??"—"}</b></p></section>
