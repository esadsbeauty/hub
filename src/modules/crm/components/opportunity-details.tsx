...
type Props = {
  ...
  onEditContact: () => void;
  onSaveValue: (value: number) => Promise<void>;
};
...
export function OpportunityDetails(props: Props) {
  ...
  return <><Drawer open={props.open} title="Detalhe da oportunidade" onClose={props.onClose}><div className="space-y-5 overflow-x-hidden">
    ...
    <section className="rounded-[1.25rem] border p-4"><label htmlFor="opportunity-proposal-value" className="text-xs font-bold uppercase tracking-[.14em] text-muted-foreground">Valor da proposta</label><div className="mt-3 grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto]"><OpportunityValueInput id="opportunity-proposal-value" value={proposalValue} onChange={setProposalValue} disabled={savingValue}/><Button className="min-h-11" disabled={savingValue||proposalValue===opportunity.value} onClick={async()=>{setSavingValue(true);try{await props.onSaveValue(proposalValue)}finally{setSavingValue(false)}}}>{savingValue?"Salvando...":"Salvar valor"}</Button></div><p className="mt-2 text-xs text-muted-foreground">Este valor alimenta o Kanban, o forecast e os indicadores comerciais.</p></section>
    <section className="rounded-[1.25rem] border p-4">
      <h3 className="text-xs font-bold uppercase tracking-[.14em] text-muted-foreground">Próxima ação</h3>
      <div className="mt-3">
        <NextActionStatus opportunity={opportunity} stages={stages} task={nextTask}/>
      </div>
      {requiresNextAction&&nextTask&&<>
        <div className="mt-4 flex flex-wrap gap-2">
          <Button size="sm" onClick={props.onCompleteNextTask}><Check size={17}/>Concluir</Button>
          <Button size="sm" variant="outline" onClick={()=>setRescheduling(value=>!value)}><RefreshCw size={16}/>Reagendar</Button>
          <Button size="sm" variant="outline" onClick={props.onAddFollowUp}><CalendarClock size={17}/>Nova próxima ação</Button>
        </div>
        {rescheduling&&<div className="mt-3 flex gap-2"><Input type="datetime-local" aria-label="Nova data do follow-up" value={newDueAt} onChange={event=>setNewDueAt(event.target.value)}/><Button disabled={!newDueAt} onClick={()=>{props.onRescheduleNextTask(new Date(newDueAt).toISOString());setRescheduling(false)}}>Salvar</Button></div>}
      </>}
      {requiresNextAction&&!nextTask&&<Button className="mt-3" variant="outline" onClick={props.onAddFollowUp}><CalendarClock size={17}/>Definir próxima ação</Button>}
    </section>
