...
const optionalPhone = z.string().trim().refine((value) => !value || value.replace(/\D/g, '').length >= 8, 'Informe um telefone válido').optional();

export const companySchema = z.object({
  fantasyName: z.string().trim().min(2, 'Informe o nome fantasia'), legalName: optionalText, cnpj: optionalText, phone: optionalPhone, whatsapp: optionalPhone,
...
export const contactSchema = z.object({ name: z.string().trim().min(2, 'Informe o nome'), role: optionalText, phone: optionalPhone, whatsapp: optionalPhone, email: optionalEmail, instagram: optionalText, linkedin: optionalText, birthDate: optionalText, notes: optionalText, isPrimary: z.boolean(), isFinancial: z.boolean(), isCommercial: z.boolean(), status: z.enum(['ativo', 'inativo']) });
...
export const opportunitySchema = z.object({
  companyId: z.string().min(1, 'Selecione a empresa'), pipelineId: z.string().min(1, 'Selecione o pipeline'), stageId: z.string().min(1, 'Selecione a etapa'),
  title: z.string().trim().min(2, 'Informe o título').max(120, 'Use até 120 caracteres'), description: optionalText,
  value: z.number().min(0, 'Informe um valor válido'), probability: z.number().min(0).max(100), expectedCloseDate: optionalText,
  owner: optionalText, source: optionalText,
});
export type OpportunityFormData = z.infer<typeof opportunitySchema>;

export const lostOpportunitySchema = z.object({
  reason: z.enum(['price', 'no_response', 'no_interest', 'competitor', 'timing', 'no_budget', 'unqualified', 'other']),
