...
export const localBlogRepository:BlogRepository={
 ...
 async getPublished(slug){return read().posts.find(p=>p.slug===slug&&p.status==="published")??null},async listCategories(){return read().categories},async listManagedCategories(){return read().categories},async listManaged(){return read().posts},
