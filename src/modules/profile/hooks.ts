...
const withUrl = (value: ProfileRow): UserProfile => {
  const path = value.avatar_path ?? undefined;
  const publicUrl =
...
export function useCurrentUserProfile() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const result = await supabase.rpc("current_user_profile");
...
export function useProfileActions() {
  ...
  const save = async (name: string, avatarPath?: string) => {
    if (!user) {
      throw new Error("Usuário não autenticado.");
    }
    ...
    if (isLocalMode || !supabase) {
      ...
      client.setQueryData(key(user.id), value);
      return value;
    }

    const result = await supabase.rpc("update_own_profile", {
      profile_name: name,
      next_avatar_path: avatarPath ?? null,
    });
    ...
    const value = withUrl(result.data as ProfileRow);
    client.setQueryData(key(user.id), value);
  ...
  return {
    ...
    changeEmail: useMutation({
      mutationFn: async (email: string) => {
        ...
        if (isLocalMode || !supabase) {
          const current = client.getQueryData<UserProfile>(key(user.id));
    ...
    uploadAvatar: useMutation({
      mutationFn: async ({
        ...
        if (!user || !supabase || isLocalMode) {
          throw new Error("O envio de foto exige conexão com o Supabase.");
        }
        ...
        const value = await save(name, path);
    ...
    removeAvatar: useMutation({
      mutationFn: async ({
        ...
        if (path && supabase && !isLocalMode) {
          const removal = await supabase.storage.from("avatars").remove([path]);
