...
const STORAGE = "esads-hub-local-v1:customers";
const LEGACY_STORAGE = "esads_customer_data_v1";
const ORG = "local-esads-beauty";
const USER = "local-owner";
const now = () => new Date().toISOString();
const id = () => crypto.randomUUID();
const empty = (): CustomerData => ({ accounts: [], services: [], customerServices: [], onboardings: [], steps: [], contracts: [] });
const read = () => { const raw = localStorage.getItem(STORAGE) ?? localStorage.getItem(LEGACY_STORAGE); return raw ? JSON.parse(raw) as CustomerData : empty(); };
