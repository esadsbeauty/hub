...
import { isOpenStage, nextActionForOpportunity } from "@/modules/crm/next-action";
...
export function DashboardPage() {
  ...
  const openWithoutAction=crm.data.opportunities.filter(opportunity=>isOpenStage(opportunity,crm.data.stages)&&!nextActionForOpportunity(opportunity,crm.data.tasks)).length;
  ...
  return (
    <PageContainer>
      ...
      <div className="flex items-center gap-2">
        ...
        <button onClick={() => setMobileFiltersOpen(true)} className="ml-auto inline-flex min-h-12 items-center gap-2 rounded-xl border border-border/70 bg-card px-3.5 text-sm font-semibold md:hidden"><SlidersHorizontal size={17}/> Filtros</button>
      ...
      {mobileFiltersOpen && <div className="fixed inset-0 z-50 bg-black/45 backdrop-blur-[2px] md:hidden" onClick={() => setMobileFiltersOpen(false)}><section role="dialog" aria-modal="true" aria-label="Filtros do dashboard" className="absolute inset-x-0 bottom-0 max-h-[88dvh] overflow-auto rounded-t-[2rem] bg-card px-5 pt-3 pb-[max(1.5rem,env(safe-area-inset-bottom))]" onClick={(event)=>event.stopPropagation()}><div className="mx-auto mb-5 h-1.5 w-12 rounded-full bg-border"/><div className="mb-5 flex items-center justify-between"><div><p className="text-xs font-semibold uppercase tracking-[.16em] text-muted-foreground">Dashboard</p><h2 className="mt-1 text-xl font-semibold">Filtros</h2></div><button aria-label="Fechar filtros" className="grid h-11 w-11 place-items-center rounded-xl bg-muted" onClick={()=>setMobileFiltersOpen(false)}><X size={20}/></button></div><AnalyticsFiltersBar data={crm.data} filters={filters} onChange={setFilters}/><button className="mt-5 min-h-12 w-full rounded-xl bg-primary text-sm font-semibold text-primary-foreground" onClick={()=>setMobileFiltersOpen(false)}>Aplicar filtros</button></section></div>}
      <section aria-label="Indicadores do dashboard" className="-mx-4 flex min-[430px]:-mx-5 snap-x snap-mandatory gap-3 overflow-x-auto scroll-smooth px-4 min-[430px]:px-5 pb-2 md:mx-0 md:grid md:grid-cols-2 md:overflow-visible md:px-0 xl:grid-cols-4">
        <div className="min-w-[78vw] snap-center md:min-w-0"><AnalyticsMetric label="Pipeline" value={compactCurrency(analytics.pipeline.value)} detail={`${analytics.pipeline.count} oportunidades abertas`} definition="Soma das oportunidades abertas." onClick={() => open("Pipeline aberto", analytics.pipeline.opportunities)} /></div>
        <div className="min-w-[78vw] snap-center md:min-w-0"><AnalyticsMetric label="Vendas no período" value={compactCurrency(analytics.won.value)} detail={`${analytics.won.count} negócios ganhos`} definition="Valor comercial ganho no período; não representa recebimento." comparison={variation(analytics.won.value, analytics.previousWonValue)} onClick={() => open("Vendas ganhas", analytics.won.opportunities)} /></div>
        <div className="min-w-[78vw] snap-center md:min-w-0"><AnalyticsMetric label="Follow-ups" value={String(analytics.attention.overdueFollowups.length)} detail="atrasados" definition="Follow-ups pendentes com prazo vencido." onClick={() => navigate("/agenda")} /></div>
        <div className="min-w-[78vw] snap-center md:min-w-0"><AnalyticsMetric label="Clientes" value={String(activeClients)} detail="ativos no relacionamento" definition="Empresas na etapa de relacionamento com clientes." onClick={() => navigate("/clientes")} /></div>
      </section>
      <Card className="md:hidden"><CardHeader className="flex-row items-center justify-between"><div><p className="text-[13px] font-semibold uppercase tracking-[.16em] text-muted-foreground">Prioridade</p><CardTitle className="mt-1">Agenda de hoje</CardTitle></div><Link aria-label="Criar atividade" to="/agenda?new=task" className="grid h-12 w-12 place-items-center rounded-2xl bg-champagne-soft"><CalendarPlus size={23}/></Link></CardHeader><CardContent className="space-y-3">{pendingToday.length?pendingToday.slice(0,3).map((task)=><TaskCard key={task.id} task={task} compact onComplete={()=>actions.completeTask.mutate(task.id)}/>):<div className="rounded-2xl bg-muted/55 px-4 py-6 text-center"><p className="font-semibold">Nenhuma atividade para hoje</p><p className="mt-1.5 text-[15px] leading-6 text-muted-foreground">Aproveite para organizar o próximo contato.</p><Link to="/agenda?new=task" className="mt-4 inline-flex min-h-12 items-center rounded-xl bg-card px-4 text-sm font-semibold shadow-soft">Nova tarefa</Link></div>}<Link className="flex min-h-[3.25rem] items-center justify-between border-t border-border/60 pt-3 text-[15px] font-semibold" to="/agenda">Abrir agenda <ArrowRight size={18}/></Link></CardContent></Card>
      <Link to="/agenda" className="flex min-h-20 items-center gap-3 rounded-[1.25rem] bg-primary p-4 text-primary-foreground shadow-soft md:hidden"><span className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-white/10"><Clock3 size={21}/></span><span className="min-w-0 flex-1"><b className="block text-base">Follow-ups</b><span className="text-sm leading-5 text-white/65">{overdue.length} atrasados · {pendingToday.length} para hoje · {openWithoutAction} sem próxima ação</span></span><ArrowRight size={20}/></Link>
      <Card className="md:hidden"><CardHeader><CardTitle>Pipeline resumido</CardTitle></CardHeader><CardContent className="space-y-3">{crm.data.stages.slice(0,3).map(stage=>{const count=crm.data.opportunities.filter(item=>item.stageId===stage.id&&item.status==="open"&&!item.deletedAt).length;return <div key={stage.id} className="flex min-h-12 items-center justify-between rounded-xl bg-muted/60 px-3.5"><span className="text-[15px] font-semibold">{stage.name}</span><span className="text-base text-muted-foreground">{count}</span></div>})}<Link className="flex min-h-12 items-center justify-between border-t pt-3 text-[15px] font-semibold" to="/crm">Ver pipeline <ArrowRight size={20}/></Link></CardContent></Card>
      <div className="md:hidden"><ForecastCard analytics={analytics}/></div>
      <Card className="md:hidden"><CardHeader><CardTitle>Performance</CardTitle></CardHeader><CardContent className="grid gap-4"><div><p className="text-base text-muted-foreground">Pipeline ponderado</p><p className="mt-1 text-xl font-semibold">{compactCurrency(analytics.weighted.value)}</p></div><div><p className="text-base text-muted-foreground">Win rate</p><p className="mt-1 text-xl font-semibold">{analytics.winRate===undefined?"—":percentage.format(analytics.winRate)}</p></div></CardContent></Card>
