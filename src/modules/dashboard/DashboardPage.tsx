...
export function DashboardPage() {
  ...
  return (
    <PageContainer>
      ...
      <Link to="/agenda" className="flex min-h-[5.75rem] items-center gap-4 rounded-[1.25rem] bg-primary p-5 text-primary-foreground shadow-soft md:hidden"><span className="grid h-[3.25rem] w-[3.25rem] shrink-0 place-items-center rounded-2xl bg-white/10"><Clock3 size={24}/></span><span className="min-w-0 flex-1"><b className="block text-[17px]">Follow-ups</b><span className="text-[15px] leading-5 text-white/65">{overdue.length} atrasados · {pendingToday.length} para hoje · {openWithoutAction} sem próxima ação</span></span><ArrowRight size={20}/></Link>
