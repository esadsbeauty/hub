...
export function DashboardPage() {
  ...
  return (
    <PageContainer>
      <div className="md:hidden"><p className="text-sm font-medium text-muted-foreground">Sua operação hoje</p><div className="mt-1 flex items-end justify-between gap-3"><h1 className="text-[2rem] font-semibold tracking-[-.05em]">Dashboard</h1><Link className="pb-1 text-sm font-semibold" to="/agenda">Ver agenda</Link></div></div>
      ...
      <div className="flex items-center gap-2">
        ...
        <button onClick={() => setMobileFiltersOpen(true)} className="ml-auto inline-flex min-h-11 items-center gap-2 rounded-xl border border-border/70 bg-card px-3 text-sm font-semibold md:hidden"><SlidersHorizontal size={17}/> Filtros</button>
      ...
      <Card className="md:hidden"><CardHeader className="flex-row items-center justify-between"><div><p className="text-xs font-semibold uppercase tracking-[.16em] text-muted-foreground">Prioridade</p><CardTitle className="mt-1">Agenda de hoje</CardTitle></div><Link aria-label="Criar atividade" to="/agenda?new=task" className="grid h-12 w-12 place-items-center rounded-2xl bg-champagne-soft"><CalendarPlus size={21}/></Link></CardHeader><CardContent className="space-y-3">{today.isLoading?<Skeleton className="h-32"/>:pendingToday.length?pendingToday.slice(0,3).map((task)=><TaskCard key={task.id} task={task} compact onComplete={()=>actions.completeTask.mutate(task.id)}/>):<div className="rounded-2xl bg-muted/55 px-5 py-7 text-center"><p className="font-semibold">Nenhuma atividade para hoje</p><p className="mt-1 text-sm text-muted-foreground">Aproveite para organizar o próximo contato.</p><Link to="/agenda?new=task" className="mt-4 inline-flex min-h-11 items-center rounded-xl bg-card px-4 text-sm font-semibold shadow-soft">Nova tarefa</Link></div>}<Link className="flex min-h-12 items-center justify-between border-t border-border/60 pt-3 text-sm font-semibold" to="/agenda">Abrir agenda <ArrowRight size={18}/></Link></CardContent></Card>
      <Link to="/agenda" className="flex min-h-[5.25rem] items-center gap-4 rounded-[1.25rem] bg-primary p-4 text-primary-foreground shadow-soft md:hidden"><span className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-white/10"><Clock3 size={22}/></span><span className="min-w-0 flex-1"><b className="block text-base">Follow-ups</b><span className="text-sm text-white/65">{overdue.data?.length ?? 0} atrasados · {pendingToday.filter((task)=>task.type === "follow_up").length} para hoje</span></span><ArrowRight size={20}/></Link>
      <div className="md:hidden"><PipelineOverview analytics={analytics}/></div>
