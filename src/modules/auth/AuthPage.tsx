...
export function AuthPage() {
  ...
  if (auth.appMode === "local") return <main className="grid min-h-dvh place-items-center bg-sidebar p-5 text-white"><section className="w-full max-w-md rounded-[1.5rem] bg-background p-8 text-center text-foreground shadow-overlay md:p-10"><span className="inline-flex rounded-full bg-champagne-soft px-3 py-1 text-xs font-semibold">Modo local</span><Sparkles className="mx-auto mt-7 text-champagne-dark" size={28}/><h1 className="mt-4 text-[2rem] font-bold tracking-[-.035em] md:text-2xl">ESADS Beauty</h1><p className="mt-2.5 text-[17px] leading-7 text-muted-foreground md:mt-2 md:text-sm md:leading-6">Acesse o Hub para testes locais. Os dados deste ambiente ficam salvos somente neste navegador.</p><Button type="button" className="mt-8 w-full" onClick={auth.enterLocalMode}>Entrar no modo local</Button></section></main>;
  return <main className="min-h-dvh bg-sidebar p-0 text-white lg:p-6"><div className="mx-auto grid min-h-dvh max-w-6xl overflow-hidden bg-sidebar lg:min-h-[calc(100dvh-3rem)] lg:grid-cols-[1.05fr_.95fr] lg:rounded-[1.75rem]"><section className="hidden flex-col justify-between p-12 lg:flex"><div><div className="mb-12 flex items-center gap-3 text-xs font-semibold tracking-[.22em]"><Sparkles className="text-champagne" size={16}/> ESADS BEAUTY</div><h1 className="max-w-lg text-5xl font-semibold tracking-[-.045em]">Sua operação, em um só lugar.</h1><p className="mt-5 max-w-md text-sm leading-6 text-white/55">Relacionamento, rotina e resultados organizados para decisões mais claras.</p></div><p className="text-xs text-white/30">Hub Interno · Acesso reservado</p></section><form onSubmit={handleSubmit(submit)} className="my-auto min-h-dvh w-full bg-background p-7 pt-16 text-foreground min-[430px]:p-8 min-[430px]:pt-16 md:p-10 lg:min-h-0 lg:max-w-md lg:rounded-[1.5rem] lg:shadow-overlay"><div className="mb-12 flex items-center justify-center gap-3 text-sm font-semibold tracking-[.2em] lg:hidden"><Sparkles className="text-champagne-dark" size={22}/> ESADS BEAUTY</div><h2 className="text-[2.125rem] font-bold leading-tight tracking-[-.035em] md:text-2xl">{title}</h2><p className="mt-2.5 text-[17px] leading-7 text-muted-foreground md:mt-2 md:text-sm md:leading-normal">Use suas credenciais para continuar.</p>{!auth.configured && <p role="alert" className="mt-5 rounded-xl bg-red-50 p-3 text-sm text-red-800">Não foi possível conectar ao serviço de autenticação.</p>}<div className="mt-9 space-y-5 md:mt-8 md:space-y-4">{!auth.passwordRecovery && <div><Label htmlFor="email">Email</Label><Input id="email" type="email" autoComplete="email" className="mt-1" {...register("email")}/></div>}{errors.email && !auth.passwordRecovery && <p className="text-sm text-red-600">Informe um email válido.</p>}{(mode === "login" || auth.passwordRecovery) && <div><Label htmlFor="password">{auth.passwordRecovery ? "Nova senha" : "Senha"}</Label><Input id="password" type="password" autoComplete={auth.passwordRecovery ? "new-password" : "current-password"} className="mt-1" {...register("password")}/></div>}<Button className="w-full" disabled={isSubmitting || !auth.configured}>{isSubmitting ? "Processando…" : auth.passwordRecovery ? "Salvar senha" : mode === "reset" ? "Enviar recuperação" : "Entrar"}</Button>{message && <p role="status" className="text-sm text-emerald-700">{message}</p>}{error && <p role="alert" className="text-sm text-red-700">{error}</p>}</div>{!auth.passwordRecovery && <div className="mt-7 text-right text-base md:mt-6 md:text-sm"><button type="button" onClick={() => setMode(mode === "login" ? "reset" : "login")}>{mode === "login" ? "Esqueci a senha" : "Voltar ao acesso"}</button></div>}{!auth.passwordRecovery && mode === "login" && auth.appMode === "supabase" && <div className="mt-8 border-t pt-7 text-center text-base md:mt-7 md:pt-6 md:text-sm text-muted-foreground">Ainda não tem acesso? <Link className="font-semibold text-foreground underline-offset-4 hover:underline" to="/register">Criar cadastro</Link></div>}</form></div></main>;
}
