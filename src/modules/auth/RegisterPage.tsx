...
export function RegisterPage() {
  ...
  if (result) return <main className="grid min-h-dvh place-items-center bg-sidebar p-5"><section className="w-full max-w-md rounded-[1.5rem] bg-background p-8 text-center shadow-overlay md:p-10"><Sparkles className="mx-auto text-champagne-dark"/><h1 className="mt-5 text-[2rem] font-bold leading-tight tracking-[-.035em] md:text-2xl">{result === "pending" ? "Cadastro recebido" : "Confirme seu email"}</h1><p className="mt-3 text-[17px] leading-7 text-muted-foreground md:text-sm md:leading-6">{result === "pending" ? "Seu acesso foi criado e está aguardando aprovação do administrador." : "Enviamos uma confirmação para seu email. Depois de confirmar, faça login para concluirmos seu acesso."}</p><Button className="mt-7 w-full" onClick={() => navigate("/login", { replace: true })}>Voltar para o login</Button></section></main>;
  return <main className="min-h-dvh bg-sidebar p-4 text-white md:grid md:place-items-center"><form onSubmit={handleSubmit(submit)} className="mx-auto min-h-[calc(100dvh-2rem)] w-full max-w-md rounded-[1.5rem] bg-background p-7 pt-12 text-foreground min-[430px]:p-8 min-[430px]:pt-12 shadow-overlay md:min-h-0 md:p-10"><div className="flex items-center justify-center gap-3 text-sm font-semibold tracking-[.2em]"><Sparkles className="text-champagne-dark" size={22}/> ESADS BEAUTY</div><h1 className="mt-9 text-[2.125rem] font-bold leading-tight tracking-[-.035em] md:mt-8 md:text-2xl">Criar cadastro</h1><p className="mt-2.5 text-[17px] leading-7 text-muted-foreground md:mt-2 md:text-sm md:leading-normal">Solicite seu acesso ao Hub Interno.</p><div className="mt-8 space-y-5 md:mt-7 md:space-y-4"><Field label="Nome" error={errors.name?.message}><Input autoComplete="name" className="md:h-11" {...register("name")}/></Field><Field label="Email" error={errors.email?.message}><Input type="email" inputMode="email" autoComplete="email" className="md:h-11" {...register("email")}/></Field><Field label="Senha" error={errors.password?.message}><Input type="password" autoComplete="new-password" className="md:h-11" {...register("password")}/></Field><Field label="Confirmar senha" error={errors.passwordConfirmation?.message}><Input type="password" autoComplete="new-password" className="md:h-11" {...register("passwordConfirmation")}/></Field>{error && <p role="alert" className="text-sm text-danger">{error}</p>}<Button className="w-full md:h-11" disabled={isSubmitting}>{isSubmitting ? "Criando cadastro…" : "Criar cadastro"}</Button></div><p className="mt-7 text-center text-base text-muted-foreground md:mt-6 md:text-sm">Já possui acesso? <Link className="font-semibold text-foreground underline-offset-4 hover:underline" to="/login">Entrar</Link></p></form></main>;
}
