...
export function DiagnosticAdminPage() {
  ...
  return <PageContainer>
    ...
    <div className="grid gap-4">{query.data?.map((item) => <Card key={item.id}><CardContent className="grid gap-5 p-5 md:grid-cols-[1.4fr_.7fr_1fr_auto] md:items-center">
      <div><div className="flex flex-wrap items-center gap-2"><h2 className="text-xl font-semibold">{item.businessName}</h2><Badge>{item.totalScore}/100</Badge></div><p className="mt-1 text-base text-muted-foreground">{item.name} · {item.whatsapp}</p><p className="text-sm text-muted-foreground">{item.email?`${item.email} · `:""}@{item.instagram}</p></div>
