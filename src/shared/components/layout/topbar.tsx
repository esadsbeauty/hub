  ...
  return <header className="sticky top-0 z-20 border-b border-border/60 bg-background/95 px-[1.125rem] pb-5 pt-[max(1rem,env(safe-area-inset-top))] min-[430px]:px-5 backdrop-blur-xl md:px-6 md:py-3 lg:px-8">
    <div className="mx-auto max-w-[90rem]">
      <div className="flex items-center gap-3">
        <div className="text-base font-bold tracking-[.2em] lg:hidden">ESADS</div>
        ...
        <div className="ml-auto flex items-center gap-1.5">
          ...
          <details className="relative"><summary className="grid h-12 w-12 cursor-pointer list-none place-items-center rounded-full bg-card shadow-soft sm:h-11 sm:w-11"><UserCircle size={23}/></summary><div className="absolute right-0 mt-2 w-64 rounded-2xl bg-card p-3 shadow-overlay"><p className="truncate text-sm font-medium">{user?.email ?? "Equipe ESADS"}</p><Link className="mt-3 block rounded-xl px-3 py-3 text-sm hover:bg-muted" to="/configuracoes">Meu perfil</Link><button className="min-h-11 w-full rounded-xl px-3 text-left text-sm text-danger hover:bg-muted" onClick={() => void signOut()}>Sair</button></div></details>
        </div>
      </div>
