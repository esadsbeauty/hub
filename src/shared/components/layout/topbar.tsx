  ...
  return <header className="sticky top-0 z-20 border-b border-border/60 bg-background/95 px-[1.125rem] pb-5 pt-[max(1rem,env(safe-area-inset-top))] min-[430px]:px-5 backdrop-blur-xl md:px-6 md:py-3 lg:px-8">
    <div className="mx-auto max-w-[90rem]">
      <div className="flex items-center gap-3">
        ...
        <div className="relative hidden flex-1 md:block md:max-w-md"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" size={17}/><Input className="border-transparent bg-muted/70 pl-10 shadow-none focus-visible:bg-card" placeholder="Buscar empresa, contato ou telefone…"/></div>
        <div className="ml-auto flex items-center gap-1.5">
          {can("crm.manage")&&<Link className="hidden h-11 items-center gap-2 rounded-xl bg-primary px-4 text-sm font-semibold text-primary-foreground md:inline-flex" to="/crm?new=company&quick=1"><Plus size={17}/> Novo</Link>}
      ...
      <form onSubmit={submitSearch} className="mt-4 md:hidden">
