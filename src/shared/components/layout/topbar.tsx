...
export function Topbar() {
  ...
  return <header className="sticky top-0 z-20 border-b border-border/60 bg-background/95 px-4 pb-4 pt-[max(.75rem,env(safe-area-inset-top))] backdrop-blur-xl md:px-6 md:py-3 lg:px-8">
    <div className="mx-auto max-w-[90rem]">
      <div className="flex items-center gap-3">
        <div className="text-sm font-bold tracking-[.2em] lg:hidden">ESADS</div>
        ...
        <div className="ml-auto flex items-center gap-1.5">
          ...
          <Button variant="ghost" size="sm" aria-label="Notificações"><Bell size={20}/></Button>
          <details className="relative"><summary className="grid h-11 w-11 cursor-pointer list-none place-items-center rounded-full bg-card shadow-soft"><UserCircle size={21}/></summary><div className="absolute right-0 mt-2 w-64 rounded-2xl bg-card p-3 shadow-overlay"><p className="truncate text-sm font-medium">{user?.email ?? "Equipe ESADS"}</p><Link className="mt-3 block rounded-xl px-3 py-3 text-sm hover:bg-muted" to="/configuracoes">Meu perfil</Link><button className="min-h-11 w-full rounded-xl px-3 text-left text-sm text-danger hover:bg-muted" onClick={() => void signOut()}>Sair</button></div></details>
        </div>
      </div>
      <form onSubmit={submitSearch} className="mt-3 grid grid-cols-[1fr_auto] gap-2 sm:hidden">
        ...
        <Link to="/crm?new=company&quick=1" className="inline-flex min-h-12 items-center gap-1.5 rounded-xl bg-primary px-4 text-sm font-semibold text-primary-foreground shadow-soft"><Plus size={18}/> Novo</Link>
      </form>
    </div>
  </header>;
}
