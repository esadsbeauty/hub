...
const linkClass = (collapsed: boolean) => ({ isActive }: { isActive: boolean }) => `group relative flex items-center rounded-xl py-2.5 text-sm font-medium transition-colors ${collapsed ? "justify-center px-2" : "gap-3 px-3"} ${isActive ? "bg-white/[.09] text-white before:absolute before:-left-1 before:h-5 before:w-0.5 before:rounded-full before:bg-champagne" : "text-white/55 hover:bg-white/[.055] hover:text-white/90"}`;

function Tooltip({ label }: { label: string }) {
  return <span role="tooltip" className="pointer-events-none absolute left-full z-50 ml-3 hidden whitespace-nowrap rounded-lg bg-white px-2.5 py-1.5 text-xs font-semibold text-sidebar shadow-overlay group-hover:block group-focus-visible:block">{label}</span>;
}

export function SidebarNavigation({ collapsed = false, onNavigate }: { collapsed?: boolean; onNavigate?: () => void }) {
  ...
  const renderLink = ({ to, label, icon: Icon, permission }: (typeof navigationItems)[number]) => can(permission) && <NavLink end={to === "/"} aria-label={collapsed ? label : undefined} title={collapsed ? label : undefined} onClick={onNavigate} key={to} to={to} className={linkClass(collapsed)}><Icon className="shrink-0" size={18} strokeWidth={1.7}/>{collapsed ? <Tooltip label={label}/> : <span>{label}</span>}</NavLink>;
  return <nav className="space-y-1" aria-label="Navegação lateral">
    {mainBeforeMarketing.map(renderLink)}
    {can("marketing.view") && <NavGroup collapsed={collapsed} label="Marketing" icon={Megaphone} open={marketingOpen} setOpen={setMarketingOpen} active={pathname.startsWith("/marketing")}>
      <NavLink end onClick={onNavigate} to="/marketing" className={linkClass(false)}>Visão Geral / Marketing</NavLink>
      <NavLink onClick={onNavigate} to="/marketing/diagnosticos" className={linkClass(false)}><ClipboardCheck size={16}/>Diagnósticos</NavLink>
      {can("blog.view") && <NavLink onClick={onNavigate} to="/marketing/blog" className={linkClass(false)}><BookOpen size={16}/>Blog</NavLink>}
    </NavGroup>}
...
function NavGroup({ collapsed, label, icon: Icon, open, setOpen, active, children }: { collapsed: boolean; label: string; icon: typeof LayoutDashboard; open: boolean; setOpen: (value: boolean) => void; active: boolean; children: React.ReactNode }) {
  const trigger = useRef<HTMLButtonElement>(null);
  const [flyoutTop, setFlyoutTop] = useState(0);
  const toggle = () => {
    if (collapsed && trigger.current) setFlyoutTop(trigger.current.getBoundingClientRect().top);
    setOpen(!open);
  };
  return <div className="relative">
    <button ref={trigger} type="button" aria-label={collapsed ? label : undefined} aria-expanded={open} title={collapsed ? label : undefined} onClick={toggle} className={`group relative flex w-full items-center rounded-xl py-2.5 text-sm font-medium hover:bg-white/[.055] hover:text-white/90 ${active ? "bg-white/[.09] text-white" : "text-white/55"} ${collapsed ? "justify-center px-2" : "gap-3 px-3"}`}>
      <Icon className="shrink-0" size={18}/>{collapsed ? <Tooltip label={label}/> : <><span className="flex-1 text-left">{label}</span><ChevronDown size={15} className={`transition-transform ${open ? "rotate-180" : ""}`}/></>}
    </button>
    {open && <div style={collapsed ? { top: flyoutTop } : undefined} className={collapsed ? "fixed left-20 z-40 ml-2 w-60 space-y-1 rounded-xl border border-white/10 bg-sidebar p-2 shadow-overlay" : "ml-4 space-y-1 border-l border-white/10 pl-2"}>{children}</div>}
  </div>;
}
