...
begin
 ...
 with days as(select generate_series(date_trunc('day',range_start at time zone zone),date_trunc('day',(range_end-interval'1 microsecond')at time zone zone),interval'1 day')::date AS calendar_day),
 internal_occurrences as(
  select e.id,e.organization_id,'internal'::text source,e.type,e.title,e.description,e.status,e.responsible_user_id,
   ((d.calendar_day+(e.start_at at time zone zone)::time)at time zone zone)start_at,
   ((d.calendar_day+(e.start_at at time zone zone)::time)at time zone zone)+(e.end_at-e.start_at)end_at,e.all_day,e.company_id,e.contact_id,e.opportunity_id,e.recurrence,null::text priority,e.external_provider,e.external_sync_status
  from public.calendar_events e cross join days d where e.organization_id=tenant and e.status<>'cancelled'and(responsible_filter is null or e.responsible_user_id=responsible_filter)and d.calendar_day>=((e.start_at at time zone zone)::date)and(e.recurrence_until is null or d.calendar_day<=e.recurrence_until)and(
   (e.recurrence='none'and d.calendar_day=(e.start_at at time zone zone)::date)or e.recurrence='daily'or(e.recurrence='weekly'and extract(dow from d.calendar_day)=extract(dow from e.start_at at time zone zone))or(e.recurrence='weekdays'and extract(dow from d.calendar_day)::smallint=any(e.recurrence_days))or(e.recurrence='monthly'and extract(day from d.calendar_day)=extract(day from e.start_at at time zone zone)))
 ),crm_events as(
